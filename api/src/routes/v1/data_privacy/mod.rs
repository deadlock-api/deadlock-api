...
pub(crate) async fn update_row_policy(
    ...
    if protected_accounts.is_empty() {
        let drop_queries = [
            ...
            "DROP ROW POLICY IF EXISTS gdpr_protection_son ON steam_profile_observed_names",
        ];
    ...
    let policy_queries = [
        ...
        format!(
            "CREATE ROW POLICY OR REPLACE gdpr_protection_son ON steam_profile_observed_names AS RESTRICTIVE FOR SELECT USING (account_id NOT IN ({protected_accounts_list})) TO api_readonly_user, dump_user"
        ),
    ];
