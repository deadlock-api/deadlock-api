...
ARRAY JOIN arrayDistinct(items.item_id) AS cohort_item_id
WHERE match_mode IN ('Ranked', 'Unranked')
    AND start_time >= toStartOfDay(now()) - INTERVAL 42 DAY
    AND start_time < toStartOfDay(now()) - INTERVAL 35 DAY
    AND duration_s > 0
GROUP BY game_mode, day, cohort_item_id, item_id, bucket_minute
SETTINGS max_bytes_before_external_group_by = 8000000000, max_threads = 8, max_memory_usage = 16106127360, log_comment = 'item_cohort_stats_time_agg_backfill';
...
ARRAY JOIN arrayDistinct(items.item_id) AS cohort_item_id
WHERE match_mode IN ('Ranked', 'Unranked')
    AND start_time >= toStartOfDay(now()) - INTERVAL 49 DAY
    AND start_time < toStartOfDay(now()) - INTERVAL 42 DAY
    AND duration_s > 0
GROUP BY game_mode, day, cohort_item_id, item_id, bucket_minute
SETTINGS max_bytes_before_external_group_by = 8000000000, max_threads = 8, max_memory_usage = 16106127360, log_comment = 'item_cohort_stats_time_agg_backfill';
...
ARRAY JOIN arrayDistinct(items.item_id) AS cohort_item_id
WHERE match_mode IN ('Ranked', 'Unranked')
    AND start_time >= toStartOfDay(now()) - INTERVAL 56 DAY
    AND start_time < toStartOfDay(now()) - INTERVAL 49 DAY
    AND duration_s > 0
GROUP BY game_mode, day, cohort_item_id, item_id, bucket_minute
SETTINGS max_bytes_before_external_group_by = 8000000000, max_threads = 8, max_memory_usage = 16106127360, log_comment = 'item_cohort_stats_time_agg_backfill';
...
ARRAY JOIN arrayDistinct(items.item_id) AS cohort_item_id
WHERE match_mode IN ('Ranked', 'Unranked')
    AND start_time >= toStartOfDay(now()) - INTERVAL 63 DAY
    AND start_time < toStartOfDay(now()) - INTERVAL 56 DAY
    AND duration_s > 0
GROUP BY game_mode, day, cohort_item_id, item_id, bucket_minute
SETTINGS max_bytes_before_external_group_by = 8000000000, max_threads = 8, max_memory_usage = 16106127360, log_comment = 'item_cohort_stats_time_agg_backfill';
...
ARRAY JOIN arrayDistinct(items.item_id) AS cohort_item_id
WHERE match_mode IN ('Ranked', 'Unranked')
    AND start_time >= toStartOfDay(now()) - INTERVAL 65 DAY
    AND start_time < toStartOfDay(now()) - INTERVAL 63 DAY
    AND duration_s > 0
GROUP BY game_mode, day, cohort_item_id, item_id, bucket_minute
SETTINGS max_bytes_before_external_group_by = 8000000000, max_threads = 8, max_memory_usage = 16106127360, log_comment = 'item_cohort_stats_time_agg_backfill';
