...
export function UsageInstructions() {
  ...
  return (
    <Card tone="glass" size="sm" radius="lg">
      <CardContent>
        <Tabs value={tab} onValueChange={setTab} className="gap-4">
          <ResponsiveTabsList variant="line" value={tab} onValueChange={setTab} aria-label="Usage guides">
            {STATIC_TABS.map((t) => (
              <ResponsiveTab key={t.id} value={t.id}>
                {t.label}
              </ResponsiveTab>
            ))}
          </ResponsiveTabsList>
          {STATIC_TABS.map((t) => (
            <TabsContent key={t.id} value={t.id}>
              {t.id === "mcp" ? (
                ...
                <HighlightedCode code={t.code} language={t.language} actions="copy" />
              )}
            </TabsContent>
          ))}
        </Tabs>
