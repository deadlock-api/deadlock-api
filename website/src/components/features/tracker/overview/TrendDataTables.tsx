...
export function RankHistoryTable({
  ...
  if (ranks.length === 0) {
    return (
      <EmptyState
        title="No recorded ranks"
        description="No matches in this selection have a recorded rank badge. Try a different date range or mode."
      />
    );
  }
  return (
    ...
    <Table>
      ...
      <TableHeader>
        <TableRow>
          ...
          <TableHead className="hidden text-end @xs/stats-dialog:table-cell">Record</TableHead>
        ...
        {[...activity.buckets].reverse().map((bucket) => {
          ...
          return (
            <TableRow key={bucket.bucketStartUnix}>
              ...
              <TableCell className="hidden text-end tabular-nums @xs/stats-dialog:table-cell">
                {bucket.wins}W / {bucket.losses}L
              </TableCell>
              <TableCell className="text-end tabular-nums">
