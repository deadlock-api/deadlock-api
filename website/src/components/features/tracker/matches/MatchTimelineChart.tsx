...
function ObjectiveMarker({
  ...
  const color = event.own ? CHART_COLOR.positive : CHART_COLOR.negative;
    ...
    <TooltipCard>
      <TooltipHeader
        title={
          <span
            className={cn(
              point.lead > 0 ? TONE_TEXT.positive : point.lead < 0 ? TONE_TEXT.negative : "text-popover-foreground",
            )}
          >
            {point.lead === 0
              ? "Even"
...
function ChipMarker({
  ...
  const tone = event.side === "gain" ? "positive" : "negative";
