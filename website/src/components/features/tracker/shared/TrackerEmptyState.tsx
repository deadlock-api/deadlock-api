...
export function TrackerEmptyState({
  ...
  return (
    <EmptyState
      icon={hasHistory ? ChartNoAxesCombined : History}
      title={hasHistory ? "No matches fit these filters" : "No match history yet"}
      description={
        hasHistory
          ? recoveries.length > 0
        ...
        recoveries.length > 0 && (
          <>
            {recoveries.map((recovery, index) => (
              ...
              >
                {recovery.label}
                <span className="ms-auto tabular-nums">{recovery.matches.toLocaleString("en-US")}</span>
              </Button>
            ))}
