...
import { DISABLED_STATE, FOCUS_RING_BORDER, INVALID_STATE, SELECTED_STATE, SVG_SLOT } from "~/components/ui/recipes";
...
const toggleVariants = cva(
  [
    ...
    DISABLED_STATE,
    SVG_SLOT,
    "inline-flex h-8 min-w-8 items-center justify-center gap-2 rounded-md px-1.5 text-sm font-medium whitespace-nowrap transition-[color,box-shadow] hover:bg-muted hover:text-muted-foreground",
    SELECTED_STATE,
  ],
  {
    variants: {
...
function ToggleGroup({
  ...
  return (
    <ToggleGroupPrimitive.Root
      ...
      {...props}
    >
      <ToggleGroupContext.Provider value={{ variant, spacing }}>{children}</ToggleGroupContext.Provider>
    </ToggleGroupPrimitive.Root>
  );
}
