...
import type { MapMarker, MatchReplayBlock, Point, ReplayTrack } from "~/lib/coach/report";
...
function trailPoints(track: ReplayTrack, t: number): { x: number; y: number }[] {
  const s = track.samples;
  if (s.length === 0) return [];
  const start = t - TRAIL_WINDOW;
...
export function MatchReplay({ block }: { block: MatchReplayBlock }) {
  ...
  // Play the whole match in ~20 real seconds.
      ...
      <div className="grid gap-4 lg:grid-cols-[1.1fr_0.9fr]">
        ...
        <div className="flex flex-col gap-3">
          ...
          <div
            ...
            style={{ borderColor: activeAnnotation ? `${toneColor(activeAnnotation.tone ?? "warning")}55` : undefined }}
          >
            {activeAnnotation ? (
      ...
      <div className="mt-3 flex items-center gap-3">
        ...
        <div className="relative flex-1">
          ...
          <input
            ...
            style={{
              background: `linear-gradient(90deg, var(--primary) ${(t / duration) * 100}%, rgba(255,255,255,0.1) ${(t / duration) * 100}%)`,
            }}
          />
        </div>
