...
/** A RankTierTick that sizes its badge from the width of the chart it is drawn in. */
export function SizedRankTierTick(props: Omit<React.ComponentProps<typeof RankTierTick>, "size">) {
  const chartWidth = useChartWidth() ?? 0;
  return <RankTierTick {...props} size={rankTickSize(chartWidth, props.tiers.length)} />;
}
