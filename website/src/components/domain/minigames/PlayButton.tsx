...
interface PlayButtonProps extends Omit<React.ComponentProps<"button">, "children"> {
  ...
  label?: string;
  /**
   * What pressing it does while playing: `pause`, or `replay` from the start, for clips of a second or two where a
   * pause is never wanted and a second press means "again".
   */
  playingAction?: "pause" | "replay";
}
...
export function PlayButton({
  ...
  const Glyph = !playing ? Play : playingAction === "replay" ? RotateCcw : Pause;
  const action = !playing ? "Play" : playingAction === "replay" ? "Replay" : "Pause";
  return (
    ...
    <button
      ...
      aria-pressed={playingAction === "pause" ? playing : undefined}
      className={cn(
