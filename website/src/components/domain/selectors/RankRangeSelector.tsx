...
function RankIcon({ option, className }: { option: RankOption; className?: string }) {
  return (
    <span
      className={cn(
        "flex shrink-0 items-center justify-center overflow-hidden rounded-md border border-hairline bg-subtle-active",
        className,
      )}
    >
      <ImgWithSkeleton
...
> {
  value?: RankRange;
  /** The range it starts at when uncontrolled, and the one the reset returns to. */
  defaultValue?: RankRange;
  onValueChange?: (value: RankRange) => void;
}

export function RankRangeSelector({
  value: valueProp,
  defaultValue = ANY_RANK,
  onValueChange,
  className,
  ...props
}: RankRangeSelectorProps) {
  const [resetMin, resetMax] = defaultValue;
  const [[minRank, maxRank], setValue] = useControllableState<RankRange>({
    value: valueProp,
    defaultValue,
    onValueChange,
  });
  ...
  const handleValueCommit = (newValue: number[]) => {
    ...
    if (options[startIdx] && options[endIdx]) {
      setValue([options[startIdx].rankId, options[endIdx].rankId]);
    }
  };
  ...
  return (
    <FilterCell
      ...
      onReset={() => setValue([resetMin, resetMax])}
      icon={triggerIcon}
      className={className}
      ...
      <div className="grid gap-4">
        ...
        <Segmented
          ...
          onValueChange={(name) => {
            ...
            if (preset) setValue([preset.min, preset.max]);
          }}
        >
          {presets.map((p) => (
