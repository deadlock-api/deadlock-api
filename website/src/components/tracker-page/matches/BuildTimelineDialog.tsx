...
function EventImage({ event }: { event: BuildEvent }) {
  return "ability" in event ? (
    <AssetImage
      asset={{
        webp: event.ability.image_webp,
        png: event.ability.image,
        fallbackSrc: event.ability.image_webp ?? event.ability.image,
        alt: "",
        title: "",
      }}
      isLoading={false}
      emptyClassName="size-6 rounded-sm bg-muted"
      imgClassName="size-6 object-contain dark:brightness-0 dark:invert"
    />
  ) : (
    <ItemImageFromAsset item={event.item.upgrade} className="size-6 rounded-sm" title="" />
  );
}
        ...
        {visible.map((event, offset) => {
          ...
          return (
            <li key={index} className="relative min-w-0">
              <span aria-hidden="true" className="absolute inset-x-0 top-7 h-px bg-border" />
              <Button
              ...
              >
                <span className="text-[9px] leading-none tabular-nums">{formatMatchDuration(event.time)}</span>
                <span aria-hidden="true" className="relative rounded-sm bg-background">
                  <EventImage event={event} />
                  <span className="absolute -right-1 -bottom-0.5 rounded-sm bg-background p-px">
...
export function BuildTimelineDialog({ build, playerName }: { build: PlayerBuild; playerName: string }) {
  ...
  return (
    <Dialog>
      ...
      <DialogContent
        className="flex flex-col gap-2 p-3 sm:max-w-[min(80rem,calc(100%-2rem))]"
        onClick={(event) => event.stopPropagation()}
      >
