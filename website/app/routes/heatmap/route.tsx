...
export function meta() {
	return createPageMeta({
		...
		description:
			"Interactive kill heatmaps showing combat hotspots across Deadlock maps.",
		path: "/heatmap",
	});
}
...
export default function Heatmap() {
	const [viewMode, setViewMode] = useQueryState(
		"view",
		parseAsStringLiteral(VIEW_MODES).withDefault("kills"),
	);
	const [team, setTeam] = useQueryState("team", parseAsInteger.withDefault(0));
	const [is3D, setIs3D] = useQueryState(
		"3d",
	...
	const [minGameTime, setMinGameTime] = useQueryState(
		"min_game_time",
		parseAsInteger.withDefault(0),
	);
	const [maxGameTime, setMaxGameTime] = useQueryState(
		"max_game_time",
		parseAsInteger.withDefault(3600),
	);
	const [sensitivity, setOutlierSensitivity] = useQueryState(
		"outlier",
		parseAsInteger.withDefault(9900),
	);
		...
		<div className="flex h-[calc(100dvh-2rem)] flex-col gap-4">
			<div className="shrink-0 text-center">
				...
				<p className="mt-1 text-sm text-muted-foreground">
					Visualize kill and death hotspots across the map
				</p>
			</div>

			<Filter.Root>
				<Filter.Team value={team} onChange={setTeam} />
				<Filter.HeatmapViewMode
					value={viewMode}
							...
							<Suspense fallback={<LoadingLogo />}>
								<Heatmap3D
									...
									onSensitivityChange={(v) =>
										setOutlierSensitivity(Math.round(v * 10000))
									}
								/>
							</Suspense>
						</ChunkErrorBoundary>
