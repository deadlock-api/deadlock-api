...
collapsed section at the end explains how, and why you might not trust it. The sample is every captured ranked match
from launch on 30 July through 6 September 2026, minus players still in calibration, the first ranked games a player
plays before the system has settled on their rank: 641,345 matches and 248,636 players. Calibrating players lose more
than average, so the players who remain win slightly more than half their games as a group. A player counted as solo can
still have a duo among the other five on their team. And none of this is an experiment: people choose when, and with
whom, to queue.
...
rank at the time, and it is where our duo detection is least reliable, so treat it as roughly right.

From the solo player's seat: in 66,018 matches where exactly one team had a duo, the duo side won 51.3%, so facing a duo
without one costs you about one extra loss in a hundred games. In the Ascendant and Eternus group it is 54.6%, about one
in twenty.
...
Duo games are a minority everywhere, from 14% of games at Initiate to 27% at Phantom and 24% in the Ascendant and
Eternus group. Both members of a duo count, so a team with one duo and four solo players is 33% duo games. Seen from the
team, duos are common: about half of all teams contain at least one, 43% at Initiate rising to 65% at Oracle and
Phantom, and two thirds of matches have a duo on at least one side. The players behind them are a different story. Below
Emissary, 40% to 49% of players have at least one detected duo game. That rises to 61% at Oracle, 84% at Phantom, and
148 of the 151 players whose usual rank is Ascendant or Eternus. Nearly everyone at the top has duoed, but they still
mostly queue alone.
...
both accounts already had 200 or more games before ranked won 53.6% together, and the 239 pairs with a brand-new account
won 50.7%.

These are different pairs, not the same pair over time. Within a pair the edge shrinks fast. Following only the 438
no-history pairs that reached twenty games together, the win rate fell from 62.4% in games 1 to 5 to 54.5% in games 16
to 20. Pairs that keep queueing are a self-selected group, and we cannot separate matchmaking, stronger opponents, who
keeps queueing, or simply duoing at different hours and in a different mood than soloing.
...
while a loss cost 207. So the whole ladder drifts upward: players who never queued as a duo still climbed **5.8 subranks
...
slope differs a lot by rank, and the same arithmetic gives anywhere from one and a half to five subranks, so read it as
an order of magnitude. Long-standing pairs never show that premium: players whose most frequent partner accounts for
more than 60% of their pre-ranked games won 51.2% solo and 50.3% together.
...
about 4 points of win rate and an extra subrank or so per 100 games, while a partner you already have hundreds of games
with is worth nothing measurable.
