...
export function LeaderboardSummary({ ranks, leaderboard, onBadgeClick }: LeaderboardSummaryProps) {
  ...
  const badgeStartingRanks = useMemo(() => {
    ...
    leaderboard.entries.forEach((entry) => {
      if (entry.badge_level && !ranksByBadge.has(entry.badge_level) && entry.rank) {
