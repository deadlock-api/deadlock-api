...
export default function HeroMatchupStatsTable({
  ...
  const { data: synergyData, isLoading: isLoadingSynergy } = useQuery<APIHeroSynergyStats[]>({
    queryKey: ["api-hero-synergy-stats", heroId, minRankId, maxRankId, minDateTimestamp, maxDateTimestamp],
    queryFn: async () => {
      const url = new URL(`https://api.deadlock-api.com/v1/analytics/hero-synergy-stats/${heroId}`);
  ...
  const { data: counterData, isLoading: isLoadingCounter } = useQuery<APIHeroCounterStats[]>({
    queryKey: ["api-hero-counter-stats", heroId, minRankId, maxRankId, minDateTimestamp, maxDateTimestamp],
    queryFn: async () => {
      const url = new URL(`https://api.deadlock-api.com/v1/analytics/hero-counter-stats/${heroId}`);
      url.searchParams.set("min_average_badge", (minRankId ?? 0).toString());
      url.searchParams.set("max_average_badge", (maxRankId ?? 116).toString());
      if (minDateTimestamp) url.searchParams.set("min_unix_timestamp", minDateTimestamp.toString());
      if (maxDateTimestamp) url.searchParams.set("max_unix_timestamp", maxDateTimestamp.toString());
      ...
      if (synergy.hero_id1 === heroId) {
        synergies.push({
          ...
          rel_winrate:
            ...
            (synergy?.wins / synergy.matches_played + synergy?.wins / synergy.matches_played) / 2,
        });
      }
  ...
  const heroCounters = useMemo(() => {
    ...
    for (const counter of counterData || []) {
      if (counter.hero_id === heroId) {
        counters.push({
          ...counter,
          rel_winrate: counter?.wins / counter?.matches_played - counter?.wins / counter?.matches_played,
        });
      }
    }
